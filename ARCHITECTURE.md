...
9. **Post-Task Knowledge Loop** — After every task completion, agents must update relevant brain files, skills, and docs before closing. Nothing gets lost to context window rot. The task isn't done until the knowledge is persisted. This is what makes the system self-improving — not just the skill extraction, but the continuous updating of brains and docs.
10. **Context Window Checkpoint (50% Rule)** — At 50% context window consumed, the agent evaluates: "Is this task close to completion?" If yes → continue to completion. If no → pause, serialize current state to a handoff file, break remaining work into sub-tasks, and dispatch to fresh agents with the same skill set. This prevents [context anxiety](https://www.anthropic.com/engineering/harness-design-long-running-apps) — Anthropic's term for agents wrapping up prematurely near context limits — and keeps task quality consistent regardless of task size.
11. **Agent Handoff Protocol** — When an agent hits the context checkpoint and determines the task can't complete in the remaining window, it writes a handoff summary — not just task state, but a full session summary: what was attempted, what worked, what didn't, decisions made, relevant file paths, and exactly where to pick up. The receiving agent reads the handoff as its first action and continues without missing a beat. Key mindset: agents don't panic at context limits. They know they have infinite equally-capable instances available. The job isn't to finish — it's to either finish **or** hand off so cleanly that finishing is trivial for the next agent. A clean handoff is a successful task completion.

> **Foundation:** Principles 9–11 together form the core of a self-improving system that doesn't degrade over time. Knowledge is persisted after every task (9), context pressure never causes premature wrap-up (10), and every handoff is clean enough that the next agent hits the ground running (11). Every run leaves the system smarter than it found it.
