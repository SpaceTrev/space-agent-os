...
export default function WorkspaceLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-full">
      {/* Workspace sidebar */}
      <aside className="w-48 flex-shrink-0 flex flex-col bg-surface-base border-r border-outline-variant">
        {/* Workspace header */}
        <div className="px-3 py-3 border-b border-outline-variant">
          <Link
            href="/dashboard"
            className="flex items-center gap-1.5 text-xs text-on-surface-variant hover:text-on-surface transition-colors mb-2"
          >
            <ChevronLeft className="w-3.5 h-3.5" />
            All workspaces
          </Link>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded bg-surface-highest flex items-center justify-center text-xs font-bold text-on-surface">
